...
# Synopisis
This report investigates data from the NOAA Storm Database to identify the type of storms that cause the greatest impact to both population health and economic damage. The data examined is from 1950 to 2011; in the earlier years of this dataset there are generally fewer events recorded. However, no attempts were made to compensate for the fewer recorded events.

According to the data that was accesssed, and this analysis, the worst impacts are: 
...
The Storm Data may be found at the course website Coursera Reproducible Research: [storm data](https://d396qusza40orc.cloudfront.net/repdata%2Fdata%2FStormData.csv.bz2)

For more information available from the [National Centers for Environmental Information](https://www.ncdc.noaa.gov/stormevents/):
...
According to the National Weather Service Storm Data Documentation, _2.1.1 Storm Data Event Table_, there are 48 event types (EVTYPE).

> `r tufte::quote_footer('--- National Weather Service Storm Data Documentation - Section2.1.1 - STORM DATA EVENT TABLE')`
...
The package _stringdist::amatch()_ was used in an attempt to find the approximate match.
Note that the parameters of amatch() used were selected simply following suggestions in Stack Overflow [String Matching](https://stackoverflow.com/questions/22371212/string-matching-using-stringdist-and-amatch-in-r). Other parameters were attempted but not much effort spent in trying to optimize this process.
...
> `r tufte::quote_footer('--- National Weather Service Storm Data Documentation - Section2.7 - STORM DATA PREPARATION')`
...
ggplot(data=populationHealth, 
  ...
  ggtitle("Population Health Inflicted by Storms") +
  xlab("Storm Type") +
...
ggplot(data=econDamage, 
  ...
  geom_bar(stat="identity") + 
  theme(axis.text.x = element_text(angle=90)) + 
  ggtitle("Economic Damage Inflicted by Storms") +
  xlab("Storm Type") +
